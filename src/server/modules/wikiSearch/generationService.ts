...
export const generateAnswerFromContext = async (
  ...
  if (contextChunks.length === 0) {
    ...
    return {
      answer:
        "I couldn't find specific information in the provided documents to answer that query.",
      sources: [],
    };
  }
  ...
  const contextString = contextChunks
    .map(
      (chunk, index) =>
        `Context Chunk [${index + 1}] (Source: ${chunk.source || 'Unknown'}):\n${chunk.content}`
    )
    .join('\n\n---\n\n');
...
Answer:`;
  ...
  try {
    ...
    const fallbackAnswer =
      "I'm unable to generate a comprehensive answer at the moment. Based on the available context, please refer to the source documents for more information.";
    ...
    const mentionedSources = uniqueSources.filter(
      (source) => source && generatedAnswer.toLowerCase().includes(source.toLowerCase())
    );

    // If no sources explicitly mentioned, include all that were provided as context
    const sourcesToCite = mentionedSources.length > 0 ? mentionedSources : uniqueSources;
    ...
    return {
      answer:
        'I encountered an error while generating an answer. Please try again or refer to the source documents directly.',
