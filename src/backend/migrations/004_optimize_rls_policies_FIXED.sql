-- Optimize RLS policies for better performance (FIXED VERSION)
-- Replace auth.uid() with (SELECT auth.uid()) to prevent re-evaluation per row
-- FIXED: Removed audio_recordings policies since it doesn't have user_id column
...
-- ============================================
-- AUDIO_RECORDINGS POLICIES (Session-based)
-- ============================================
-- Note: audio_recordings doesn't have user_id, it uses session_id

DROP POLICY IF EXISTS "Users can view recordings from their sessions" ON audio_recordings;
CREATE POLICY "Users can view recordings from their sessions" ON audio_recordings
    FOR SELECT
    TO authenticated
    USING (
        ...
        EXISTS (
            ...
            AND (debate_sessions.user_id = (SELECT auth.uid()) OR debate_sessions.user_id IS NULL)
        )
    );

DROP POLICY IF EXISTS "Users can create recordings in their sessions" ON audio_recordings;
CREATE POLICY "Users can create recordings in their sessions" ON audio_recordings
    FOR INSERT
    TO authenticated
    WITH CHECK (
        session_id IS NULL OR
...
DROP POLICY IF EXISTS "Users can update recordings in their sessions" ON audio_recordings;
CREATE POLICY "Users can update recordings in their sessions" ON audio_recordings
    FOR UPDATE
    TO authenticated
    USING (
        session_id IS NULL OR
...
DROP POLICY IF EXISTS "Users can delete recordings from their sessions" ON audio_recordings;
CREATE POLICY "Users can delete recordings from their sessions" ON audio_recordings
    FOR DELETE
    TO authenticated
    USING (
        session_id IS NULL OR
...
-- VERIFICATION QUERY
-- ============================================
-- Run this after migration to verify optimization:
/*
SELECT 
    schemaname,
    tablename,
...
WHERE schemaname = 'public'
    AND tablename IN (
        'user_profiles', 'saved_searches', 'speech_feedback', 
        'audio_recordings', 'debate_speeches', 'debates', 
        'debate_feedback', 'speech_recordings', 'debate_sessions'
    )
