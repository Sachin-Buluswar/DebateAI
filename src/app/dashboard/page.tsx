...
import { MicrophoneIcon, ChatBubbleLeftRightIcon, AcademicCapIcon, MagnifyingGlassIcon, ClockIcon, FireIcon } from '@heroicons/react/24/solid';
import {
...
export default function Dashboard() {
  ...
  const [weeklyChartData, setWeeklyChartData] = useState<{ name: string; speeches: number; debates: number; }[]>([]);
      ...
      try {
        ...
        try {
          ...
          if (speechError && speechError.code !== '42P01') {
            ...
            setSpeechHistory([...fetchedSpeeches].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())); 
            
            // Calculate stats from speech feedback
            if (fetchedSpeeches.length > 0) {
              // Hours spent estimate
              const speechHours = fetchedSpeeches.reduce((sum, speech) => sum + (speech.duration_seconds ? speech.duration_seconds / 3600 : 0.25), 0);
  ...
  return (
    <ErrorBoundary fallback={
      ...
      <DashboardLayout>
          ...
          <div className="col-span-4 bg-red-50 dark:bg-red-900/30 border border-red-300 dark:border-red-700 rounded-md p-4 mb-6">
            <div className="flex">
              <div className="flex-shrink-0">
                <svg className="h-5 w-5 text-red-500 dark:text-red-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"> {/* Updated class */} 
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                </svg>
              </div>
              <div className="ml-3">
                <p className="text-sm text-red-700 dark:text-red-300 font-medium">{error}</p> {/* Updated class */} 
              </div>
            </div>
          </div>
        )}
        ...
        <Widget title="Practice Stats" className="col-span-4 md:col-span-2 xl:col-span-1">
          <div className="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
              ...
              <p className={`text-3xl font-bold ${avgScores.count > 0 ? 'text-yellow-500' : 'text-gray-400'}`}>
        ...
        <Widget title="Overall Learning Progress" className="col-span-4 md:col-span-2 xl:col-span-1">
          {avgScores.count > 0 ? (
            <div className="space-y-4">
              <ProgressItem 
               ...
               />
              <p className="text-xs text-gray-500 dark:text-gray-400 text-center">Based on {avgScores.count} analyzed speech{avgScores.count !== 1 ? 'es' : ''}.</p>
            </div>
          ) : (
            <p className="text-center text-gray-500 dark:text-gray-400 py-4">No speech scores available yet. Practice a speech to see your progress!</p>
          )}
        </Widget>
        ...
        <Widget title="Highlights & Milestones" className="col-span-4 md:col-span-2 xl:col-span-1">
          <div className="space-y-4">
            <div className="flex items-center justify-between p-3 bg-gradient-to-r from-yellow-50 to-amber-100 dark:from-yellow-900/30 dark:to-amber-900/40 rounded-lg">
              <div>
                <p className="text-sm font-medium text-amber-800 dark:text-amber-200">Highest Score</p>
                <p className={`text-2xl font-bold ${highestScore !== null ? 'text-amber-600 dark:text-amber-400' : 'text-gray-400 dark:text-gray-500'}`}>
            ...
            <div className="flex items-center justify-between p-3 bg-gradient-to-r from-sky-50 to-cyan-100 dark:from-sky-900/30 dark:to-cyan-900/40 rounded-lg">
               <div>
                <p className="text-sm font-medium text-cyan-800 dark:text-cyan-200">Total Practice Time</p>
        ...
        <Widget title="Overall Score Trend" className="col-span-4 md:col-span-2">
          {scoreTrendData.length > 1 ? (
            ...
            <p className="text-center text-gray-500 dark:text-gray-400 py-4">Not enough data to show score trend. Complete at least two scored speeches.</p>
          )}
        </Widget>
        ...
        <Widget title="Recent Activity" className="col-span-4">
          <div className="space-y-4">
            {recentActivity.length > 0 ? (
              recentActivity.map(item => {
                ...
                return (
                  <Link href={href} key={item.id} className="block p-3 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center min-w-0">
                         <div className={`flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center mr-3 ${isSpeech ? 'bg-teal-100 dark:bg-teal-700' : 'bg-primary-100 dark:bg-primary-700'}`}>
                           {isSpeech ? (
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-medium text-gray-900 dark:text-white truncate" title={title}>
                            {title}
                          </p>
          ...
          { (speechHistory.length > 0 || debateHistory.length > 0) &&
             <div className="mt-4 text-center">
                <Link href="/history" className="text-sm font-medium text-primary-600 dark:text-primary-400 hover:underline">
                  View all history
                </Link>
              </div>
          }
...
const WeeklyActivityChart = ({ data }: { data: { name: string; speeches: number; debates: number; }[] }) => {
  return (
    <div className="h-60 md:h-72 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart 
        ...
        >
          ...
          <Bar dataKey="speeches" fill="#14b8a6" /* teal-500 */ name="Speeches" radius={[4, 4, 0, 0]}>
             {/* Optional: Add labels inside bars if needed */}
             {/* <LabelList dataKey="speeches" position="top" fontSize={10} fill="#fff" /> */}
           </Bar>
          <Bar dataKey="debates" fill="#6366f1" /* primary-500 */ name="Debates" radius={[4, 4, 0, 0]}>
