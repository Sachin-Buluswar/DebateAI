  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const searchEndpoint =
        searchMode === 'rag' ? '/api/wiki-document-search' : '/api/wiki-search';
          ...
          {results.length > 0 && (
            <>
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="px-3 py-1 text-xs font-medium rounded-full bg-primary-100 text-primary-700 dark:bg-primary-800 dark:text-primary-200">
                  {searchMode === 'rag' ? 'Document Search' : 'AI-Enhanced'}
                </span>
              </div>
              {results.map((res, idx) => (
                <UnifiedSearchCard key={idx} result={res} index={idx} searchMode={searchMode} />
              ))}
            </>
          )}
          {hasSearched && !searching && results.length === 0 && !error && (
            <div className="text-center py-12">
              ...
              <p className="mt-2 text-sm text-gray-500 dark:text-gray-400 max-w-md mx-auto">
                No documents matched your search for &ldquo;{query}&rdquo;. Try different keywords
