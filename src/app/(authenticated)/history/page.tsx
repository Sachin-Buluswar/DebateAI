...
const HistoryAudioPlayer = memo(({ audioUrl }: { audioUrl: string }) => {
  ...
  return (
    <div className="mt-3 bg-gray-50 dark:bg-gray-700 p-3 rounded-md">
      {error ? (
        <div className="text-red-500 dark:text-red-400 py-1 text-xs">{error}</div>
      ) : loading ? (
        ...
        <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg hover:shadow-lg transition-shadow duration-200 animate-fade-in">
          ...
          <div className="px-4 py-4 sm:px-6 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
            <div className="flex items-center min-w-0 flex-1">
              <div
                className={`flex-shrink-0 h-8 w-8 sm:h-10 sm:w-10 rounded-full flex items-center justify-center ${isSpeech ? 'bg-accent-100 dark:bg-accent-700' : 'bg-primary-100 dark:bg-primary-700'}`}
              >
                {isSpeech ? (
              ...
              <div className="ml-3 min-w-0 flex-1">
                <h3
                  className="text-sm sm:text-base font-medium text-gray-900 dark:text-gray-100 break-words"
                  title={title}
                >
                  {title}
                </h3>
                ...
                <div className="ml-3 flex-shrink-0">
                  {(() => {
                    ...
                    if (score !== null) {
                      ...
                      const bgColorClass = scoreColorClass.includes('green')
                        ? 'bg-green-50 dark:bg-green-900/20'
                        : scoreColorClass.includes('blue')
                          ? 'bg-blue-50 dark:bg-blue-900/20'
                          : scoreColorClass.includes('yellow')
                            ? 'bg-yellow-50 dark:bg-yellow-900/20'
                            : 'bg-red-50 dark:bg-red-900/20';
                      return (
                        <div
                          className={`px-3 py-1 rounded-full ${scoreColorClass} ${bgColorClass} font-semibold text-sm`}
                        >
                          {scoreInfo.display}
                        </div>
                      );
...
export default function History() {
  ...
  const handleDeleteItem = useCallback(async () => {
    ...
    try {
      ...
      if (type === 'speech') {
          ...
          try {
            ...
            if (storageIndex !== -1 && pathParts.length > storageIndex + 4) {
              ...
              if (bucketName && storagePath) {
                ...
                if (storageError) {
                  throw new Error(
                    `Failed to delete associated audio file: ${storageError.message}`
                  );
                }
              }
            }
        ...
        // type === 'debate'
        // Delete the record from the table
        const { error: deleteError } = await supabase.from('debate_history').delete().eq('id', id);
  ...
  const displayItems = useMemo(() => {
    if (activeTab === 'all') {
      return [...speechHistory, ...debateHistory].sort(
          ...
          {displayItems.length > 0 ? (
              ...
              <List
                ...
                onScroll={({ scrollOffset, scrollUpdateWasRequested }) => {
                  ...
                  const scrollPercentage =
                    scrollOffset / Math.max(1, displayItems.length * 200 - listHeight);
                  if (
            ...
            <div className="text-center py-12 bg-white dark:bg-gray-800 shadow-md rounded-lg">
              ...
              <h3 className="mt-2 text-lg sm:text-xl font-medium">
                No {activeTab !== 'all' ? activeTab : 'practice history'} found
              </h3>
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50 px-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 sm:p-6 shadow-xl max-w-sm w-full mx-auto">
            ...
            <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-4 sm:mb-6">
              ...
              {itemToDelete.type === 'speech' ? 'speech' : 'debate'} record? Associated audio files
              (if any) will also be removed. This action cannot be undone.
            </p>
