...
export function Widget({ title, children, className = '', colSpan = 'col-span-1' }: WidgetProps) {
  return (
    <div
      className={`bg-white dark:bg-gray-800 rounded-lg shadow-sm hover:shadow-lg transition-shadow duration-200 overflow-hidden ${colSpan} ${className}`}
    >
...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">{children}</div>
    </div>
  );
}
