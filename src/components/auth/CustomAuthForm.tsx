      ...
      if (showForgotPassword) {
        ...
        if (error) {
          ...
          if (error.message.includes('already registered')) {
            setError(
              'This email address is already associated with an account. Please sign in instead.'
            );
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      <form onSubmit={handleAuth} className="space-y-6">
        ...
        {error && (
          <div
            className="p-4 bg-red-50 dark:bg-red-900/20 border-l-2 border-l-red-500"
        ...
        {message && (
          <div
            className="p-4 bg-green-50 dark:bg-green-900/20 border-l-2 border-l-[#87A96B]"
            style={{ borderRadius: 0 }}
          >
            <div className="flex items-start gap-3">
              <svg
                className="w-5 h-5 text-[#87A96B] flex-shrink-0 mt-0.5"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
